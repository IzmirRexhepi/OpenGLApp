...
void Shader::Validate() {
	...
	if (!result) {
		glGetProgramInfoLog(shaderID, sizeof(eLog), NULL, eLog); //getting info log, passing into the elog char array
...
void Shader::CompileProgram() {
	...
	for (size_t i = 0; i < MAX_POINT_LIGHTS; i++) { // 
		// location buffer
		char locBuff[100] = { '\0' }; //null teminator = end of string
...
void Shader::AddShader(GLuint theProgram, const char* shaderCode, GLenum shaderType) {
	...
	if (!result) {
		glGetShaderInfoLog(theShader, sizeof(eLog), NULL, eLog); //getting info log, passing into the elog char array
